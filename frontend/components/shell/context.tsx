...
const Ctx = createContext<Shell>({ pid: "", project: null, activity: [], reload: async () => {}, copilotOpen: false, setCopilotOpen: () => {} });
...
export function ShellProvider({ pid, children }: { pid: string; children: React.ReactNode }) {
  ...
  return <Ctx.Provider value={{ pid, project, activity, reload, copilotOpen, setCopilotOpen }}>{children}</Ctx.Provider>;
}
